...
def primotask(files,tokens,POS):
    ...
    print(formato.format("Numero di frasi:", len(tokens[0]), len(tokens[1])))
    print(formato.format("Numero di token:", len(POS[0]), len(POS[1])))
    print()
...
def secondotask(files,tokens, POS):
    ...
    print(formato.format("",files[0],files[1]))
    print(formato.format("Lunghezza media delle frasi:", Mfrasi[0], Mfrasi[1]))
    print(formato.format("Lunghezza media dei token:", Mtokens[0], Mtokens[1]))
    print()
...
def terzotask(files, POS):
    ...
    for i in range(len(files)):
        ...
        H.append(h)
    print(formato.format("", files[0], files[1]))
    print(formato.format("Numero di hapax sui primi 1000 token:", H[0], H[1]))
    print()
...
def quartotask(file, POS):
    ...
    formato = "{:<15} {:<15} {:<15}"
    print(file)
    print(formato.format("N° di token:","Grandezza di V:","TypeTokenRatio:"))
    while c < C:
...
def quintotask(files,POS):
    ...
    formato = "{:<30} {:<20} {:<20}"
    print(formato.format("",files[0],files[1]))
    print(formato.format("Percentuale di parole piene:", PIENE[0], PIENE[1]))
    print(formato.format("Percentuale di parole vuote:", VUOTE[0], VUOTE[1]))
    print()
