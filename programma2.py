...
def primotask(files,POS, bigrammi, trigrammi):
    ...
    print(formato.format(files[0], "", files[1], ""))
    print(d)
    print(formato.format("POS", "Frequenza", "POS", "Frequenza"))
    for i in range(10):
    ...
    print(formato.format(files[0], "", files[1], ""))
    print(d)
    print(formato.format("BigrammiPOS", "Frequenza", "BigrammiPOS", "Frequenza"))
    for i in range (10):
    ...
    print(formato.format(files[0], "", files[1], ""))
    print(d)
    print(formato.format("TrigrammiPOS","Frequenza","TrigrammiPOS","Frequenza"))
    for i in range(10):
    ...
    for n in range(len(files)):
        print(files[n])
        print(d)
        print("{:<15} {:<15} {:<15} {:<15}".format("Aggettivo","Frequenza","Avverbio","Frequenza"))
        for i in range(20):
            print("{:<15} {:<15} {:<15} {:<15}".format(Ag[n][i][0][0], Ag[n][i][1], Av[n][i][0][0],Av[n][i][1]))
...
def secondotask(files, bigrammi, tokens_POS):
        ...
        for bigramma in set(bigrammi[n]):
            ...
            if (bigramma[0][1]in agg and bigramma[1][1] in nomi) or (bigramma[0][1]in nomi and bigramma[1][1] in agg):
                ...
                if freq_b_0>3 and freq_b_1>3:
                    ...
                    LMI=freq*math.log(freq/prob_cond,2)
    ...
    for n in range(len(files)):
        print(files[n])
        print("--------------------------------------------------------------------------------------------------------------------------")
        print(formato.format("Bigramma", "Frequenza","Bigramma","ProbCondizionata","Bigramma", "LMI"))
        for i in range(20):
...
def terzotask(files,tokens,POS, bigrammi, trigrammi):
    ...
    for n in range(len(files)):
        ...
        print(formato.format("Frase", "Frequenza media"))
        for frase in sorted(listafrasi,key=lambda item:item[1], reverse=True):
            frase_str=""
        ...
        print(formato.format("Frase", "Frequenza media"))
        for frase in sorted(listafrasi, key=lambda item: item[1], reverse=False):
            frase_str = ""
            for token in frase[0]:
                frase_str += str(token) + " "
            print(formato.format(frase_str, frase[1]))
        ...
        print(formato.format("Frase", "Probabilità"))
        
        #--------------ordinamento delle frasi per probabilità------------
        for frase in sorted(listafrasi_, key=lambda item: item[1], reverse=True):
            frase_str = ""
            for token in frase[0]:
                frase_str += str(token) + " "
            print(formato.format(frase_str, frase[1]))
...
def quartotask(files, NER):
    ...
    print(formato.format(files[0], "", files[1], ""))
    print("--------------------------------------------------------------------------------------")
    print(formato.format("Persona", "Frequenza","Persona", "Frequenza"))
    for i in range(15):
